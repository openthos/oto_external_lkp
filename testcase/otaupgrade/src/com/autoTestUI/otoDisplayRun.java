...
public class otoDisplayRun extends UiAutomatorTestCase{
	...
	private void SolveProblems(){
		...
		boolean authorityRequestExists = new UiObject(
				new UiSelector().resourceId("com.android.settings:id/allow")).exists();		
		if (authorityRequestExists == true) {
			System.out.println("authority request exists");
			UiObject skipButton = new UiObject(
					new UiSelector().resourceId("com.android.settings:id/allow"));
			try {
				skipButton.click();
				sleep(3000);
			} catch (UiObjectNotFoundException e) {
