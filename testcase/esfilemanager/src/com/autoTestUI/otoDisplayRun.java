...
public class otoDisplayRun extends UiAutomatorTestCase{
	...
	private void SolveProblems(){
		...
		boolean dumpFirstStart = new UiObject(
				new UiSelector().resourceId("com.estrongs.android.pop:id/title_container_common_dialog")).exists();
		if (dumpFirstStart == true) {
			UiObject skipButton = new UiObject(
					new UiSelector().resourceId("com.estrongs.android.pop:id/button_common_dialog_21"));
			try {
				skipButton.click();
			} catch (UiObjectNotFoundException e) {
				// TODO Auto-generated catch block
