...
if  [  -d $foldName  ];then
 ...
 source /etc/profile
./android3dmarkMake.sh

adb -s $androidIP:$port   push ./bin/android3dmark.jar /data/local/tmp
...
adb -s $androidIP:$port shell pm uninstall -k com.futuremark.dmandroid.application
